...
$additional = '';
if(!empty($search)) {
    $additional = 'where fullname like "%'.$search.'%" or phone_number like "%'.$search.'%" or email like "%'.$search.'%"';
}
$firstIndex = ($page - 1) * $limit;
$sql = 'select * from users '.$additional.' limit '.$firstIndex.','.$limit;
...
foreach($result as $item) {
    echo ' <tr align="center">
            ...
            <td><a onclick="deleteUser('.$item['id'].')"><i class="far fa-trash-alt"></i>Delete</a></td>
        </tr>';
}
?>
                        </tbody>
    ...
    <script>
        function deleteUser(id) {
            var option = confirm('Are you sure you want to delete this user?');
            if(!option) {
                return;
            }
            $.ajax({
